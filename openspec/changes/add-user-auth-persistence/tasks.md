...
- [ ] 1.1 User creates a Supabase project (free tier) and shares the project
      URL + anon public key; confirm Auth (email/password provider) is
      enabled by default
- [ ] 1.2 Create the `layouts` table (`user_id uuid primary key references
...
- [ ] 1.3 Enable Row Level Security on `layouts` and add policies restricting
      `select`/`insert`/`update` to `user_id = auth.uid()`; verify by reading
      the policy list back (`select * from pg_policies where tablename =
      'layouts'`)
- [ ] 1.4 User creates a Vercel account/project (free tier) linked to this
...
- [ ] 2.1 Add the `@supabase/supabase-js` CDN `<script>` tag and initialize a
      client with the project URL/anon key; verify `supabase.auth` is defined
      in the browser console
- [ ] 2.2 Implement `signUp(email, password)` and wire it to a registration
...
- [ ] 2.3 Implement `signIn(email, password)` and wire it to a login form;
      verify the "Successful login" and "Login with incorrect credentials"
      scenarios live
- [ ] 2.4 Implement `signOut()` and a visible logout control shown only when
      signed in; verify the "Successful logout" scenario live
- [ ] 2.5 Wire Supabase's session restore (`onAuthStateChange` /
      `getSession`) so sign-in state survives a reload; verify both "Reload
      while signed in" and "Reload after logout" scenarios live
- [ ] 2.6 Confirm none of the new auth DOM/handlers are attached inside
...
- [ ] 3.1 Implement the `layoutStore` seam (`layoutStore.save`/
...
- [ ] 3.2 Implement `saveLayoutForUser(user, layout)` (serialize then
...
- [ ] 3.3 Handle the unreachable-storage case with a non-blocking warning
...
- [ ] 3.4 Verify "Returning signed-in visitor with a previously saved
...
- [ ] 3.5 Verify RLS actually blocks cross-account access: attempt to read
...
- [ ] 3.6 Verify signed-out behavior is byte-for-byte unchanged: with no
...
- [ ] 4.1 Add `User` and `StoredLayoutRow` to `docs/site/ARCHITECTURE.md`'s
...
- [ ] 4.2 Add rows for `signUp`/`signIn`/`signOut`/`saveLayoutForUser`/
...
- [ ] 4.3 Update `docs/STATUS.md` and `docs/site/STATUS.md` to record the new
      auth/persistence capability and, once done, the production Vercel URL
- [ ] 4.4 Write `docs/site/reviews/review-add-user-auth-persistence.md`
